...
const EquiposController_1 = require("../../equipos/controller/EquiposController");
...
// Router from Express
let equiposRouter = express_1.default.Router();
equiposRouter.route('/')
    // GET:
    ...
    const controller = new EquiposController_1.EquipoController();
    // Get Response
    const response = yield controller.getEquipos(page, limit, id);
    ...
    const controller = new EquiposController_1.EquipoController();
    // Get Response
    const response = yield controller.deleteEquipo(id);
    // Send to the client the response
    return res.status(200).send(response);
}))
    // UPDATE
    ...
    const equipo = req.body; // Obtain the equipo data from the body
    // Controller Instance to execute a method
    const controller = new EquiposController_1.EquipoController();
    const response = yield controller.updateEquipo(id, equipo);
    if (response.success) {
        return res.status(200).send(response);
    }
    else {
    ...
    // Controller Instance to execute a method
    const controller = new EquiposController_1.EquipoController();
    const response = yield controller.createEquipo(equipoData);
    if (response.success) {
...
exports.default = equiposRouter;
//# sourceMappingURL=EquipoRouter.js.map