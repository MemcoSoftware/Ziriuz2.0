...
const RepuestosEquiposController_1 = require("../controller/RepuestosEquiposController");
...
// Router from Express
let repuestosEquiposRouter = express_1.default.Router();
repuestosEquiposRouter.route('/')
    ...
    const controller = new RepuestosEquiposController_1.RepuestoEquipoController();
    // Get Response
    const response = yield controller.getRepuestoEquipos(page, limit, id);
    ...
    const controller = new RepuestosEquiposController_1.RepuestoEquipoController();
    // Get Response
    const response = yield controller.deleteRepuestoEquipo(id);
    // Send to the client the response
    return res.status(200).send(response);
}))
    // UPDATE
    ...
    const repuestoEquipo = req.body; // Obtain the repuestoEquipo data from the body
    // Controller Instance to execute a method
    const controller = new RepuestosEquiposController_1.RepuestoEquipoController();
    const response = yield controller.updateRepuestoEquipo(id, repuestoEquipo);
    if (response.success) {
        return res.status(200).send(response);
    }
    else {
    ...
    const repuestoEquipoData = req.body;
    // Controller Instance to execute a method
    const controller = new RepuestosEquiposController_1.RepuestoEquipoController();
    const response = yield controller.createRepuestoEquipo(repuestoEquipoData);
    if (response.success) {
...
exports.default = repuestosEquiposRouter;
//# sourceMappingURL=RepuestosEquiposRouter.js.map